...
class ConfigParser
{
	...
	private:
		...
		std::string parseIndexValue(const std::vector<std::string>& tokens);
		size_t parseClientMaxBodySize(const std::vector<std::string>& tokens);
		bool isValidClientMaxBodySize(size_t size);
};
